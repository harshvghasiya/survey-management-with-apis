    ...
    { 
        if ($id ==null) {   
          ...
          $description=trans('message.new_right_add');
            ...
            PageAccess::where('right_id',$id)->delete();
        ...
        if (isset($request->page_id) && $request->page_id != NULL) {
          foreach ($request->page_id as $pa => $page) {
            if (isset($page) && $page!= null) {
                $acc=new PageAccess;
                $acc->module_id=$pa;
                $acc->right_id=$res->id;
                $acc->page_access=json_encode($page);
                $acc->save();
            }
          }
        }
    ...
    public function getListData($request)
    {
        ...
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                  ...
                  $string ='<a title="'.trans('message.edit_right').'" href="'.route('right.edit',Crypt::encrypt($data->id)).'" class="btn btn-xs btn-primary">Edit</a>';
   
                   if (env('IS_MASTER_ADMIN')==true) {
                    $string .=' <a href="javascript:void(0)" title="'.trans('message.delete_right_label').'" data-route="'.route('right.destroy',Crypt::encrypt($data->id)).'" class="btn btn-xs btn-danger delete_record">Delete</a>';
                                    }                 
