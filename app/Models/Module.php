...
class Module extends Model
{
    ...
    public function module_page_access()
    {
      return $this->hasOne('\App\Models\PageAccess','module_id','id');
    }
    ...
    public function getListData($request)
    {
        ...
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                  ...
                  $string ='<a title="'.trans('message.edit_module').'" href="'.route('module.edit',Crypt::encrypt($data->id)).'" class="btn btn-xs btn-primary">Edit</a>';

                    if (env('IS_MASTER_ADMIN')==true) {
                       $string .=' <a href="javascript:void(0)" title="'.trans('message.delete_module_label').'" data-route="'.route('module.destroy',Crypt::encrypt($data->id)).'" class="btn btn-xs btn-danger delete_record">Delete</a>';
                    }
              ...
              ->editColumn('name',function($data){
                  if (env('IS_MASTER_ADMIN')==true) {
                    $user_data= $data->name .' ( Mid -'. $data->id .' )';
                  }else{
                    $user_data=$data->name;
                  }
                  return $user_data;
